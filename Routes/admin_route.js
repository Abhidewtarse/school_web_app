...
router.post("/save_career_detail", async function (req, res) {
  ...
  //   var sql =` UPDATE career_info SET pdf_1 ='${pdf_01}' WHERE career_id =1`;
  //   var data = await exe(sql);

  // }
  //  if (req.files) {
  //   var pdf_02 = new Date().getTime() + req.files.pdf_2.name;
  //   req.files.pdf_2.mv("public/images/" + pdf_02)

  //   var sql = `UPDATE career_info SET pdf_2 ='${pdf_02}' WHERE career_id =1`;
  //   var data = await exe(sql);
...
router.get("/manage_contact", async function (req, res) {
 ...
 var obj = {"contact_list":contact_list}
